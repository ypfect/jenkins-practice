...
usage() {
  cat <<EOF
用法: $(basename "$0") --image localhost:5050/arap:1 [--env local] [--deploy-id 123] [--ci-build 1]
EOF
  exit 1
}

IMAGE=""; ENV="local"; DEPLOY_ID=""; CI_BUILD=""
while [ $# -gt 0 ]; do
  case "$1" in
...
curl -sf -b "${COOKIE_JAR}" -u "${JENKINS_USER}:${JENKINS_PASS}" \
  -H "${CRUMB_FIELD}: ${CRUMB}" \
  -X POST \
  "${JENKINS_URL}/job/arap-deploy/buildWithParameters?IMAGE=${ENCODED_IMAGE}&Env=${ENV}&deployID=${DEPLOY_ID}&CI_BUILD=${CI_BUILD}"

echo ""
echo "已提交。查看: ${JENKINS_URL}/job/arap-deploy/"
